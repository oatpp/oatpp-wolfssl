...
provider::ResourceHandle<data::stream::IOStream> ConnectionProvider::get() {
  ...
  if (sslSession == nullptr) {
    OATPP_LOGE("[oatpp::wolfssl::server::ConnectionProvider::ConnectionProvider()]", "Error. Call to wolfSSL_new() returned NULL");
    throw std::runtime_error("[oatpp::wolfssl::server::ConnectionProvider::ConnectionProvider()]: Error. Call to wolfSSL_new() failed.");
  }

  return provider::ResourceHandle<data::stream::IOStream>(
    std::make_shared<Connection>(sslSession, stream),
