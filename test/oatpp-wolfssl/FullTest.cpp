...
#include "oatpp-wolfssl/client/ConnectionProvider.hpp"
//#include "oatpp-wolfssl/configurer/TemporaryDhParamsFile.hpp"
#include "oatpp-wolfssl/server/ConnectionProvider.hpp"
...
public:
  ...
  OATPP_CREATE_COMPONENT(std::shared_ptr<oatpp::network::ServerConnectionProvider>, serverConnectionProvider)([this] {
    ...
    OATPP_LOGD("oatpp::wolfssl::Config", "pem='%s'", PKEY_PEM_PATH);
    OATPP_LOGD("oatpp::wolfssl::Config", "crt='%s'", CERT_CRT_PATH);
    //OATPP_LOGD("oatpp::wolfssl::Config", "dh_params='%s'", CERT_DH_PARAMS_PATH);

    auto config = oatpp::wolfssl::Config::createDefaultServerConfigShared(CERT_CRT_PATH, PKEY_PEM_PATH);
    //config->addContextConfigurer(
    //    std::make_shared<oatpp::wolfssl::configurer::TemporaryDhParamsFile>(CERT_DH_PARAMS_PATH)
    //);

    return oatpp::wolfssl::server::ConnectionProvider::createShared(config, streamProvider);
  }());
  ...
  OATPP_CREATE_COMPONENT(std::shared_ptr<oatpp::network::ClientConnectionProvider>, clientConnectionProvider)([this] {
    ...
    return oatpp::wolfssl::client::ConnectionProvider::createShared(config, streamProvider);
